...
public:
 ...
 while (commsLinks[0].isInUse() && commsLinks[1].isInUse()){
    stringstream s6;
    s6<< "Thread is suspended on link"<<endl;
    printing(s6.str());
    LAC_cv.wait(LAC_ul);
 }
    for (Link& vec : commsLinks)
    {
        if (!vec.isInUse()) {
            vec.setInUse();
            linkNum = vec.getLinkId();
            break;
        }
    }
...
//This function is executed by each thread:
 void run(BC& theBC, int idx, LinkAccessController& lac) {
    ...
    std::stringstream s4;
    s4<< "Thread "<<idx<<" successfully acquired the link."<<endl;
    printing(s4.str());
    ...
    std::stringstream s5;
    s5<<"Thread "<<idx<<" successfully released the link."<<endl;
    printing(s5.str());
    // delay for random period between 1 and 10 milliseconds
    std::this_thread::sleep_for(std::chrono::milliseconds((dis(gen) % 10) + 1));
